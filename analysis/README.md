...
| `spine1_as13335_degree.py` | CONTEXT.md §6n | Closes a §6m script-of-record gap: AS13335's global degree via Neo4j graph traversal (2,048/7,439 domains, 27.5%) — matches §8 exactly. Confirms the Postgres `fp_weighted`-based approach §6m's table previously speculated would work does not (`"org:AS13335"` never appears as a key; `weighted_fingerprint()` prefers `hosting_provider`). |
| `spine2_ns_pool_census.py` | CONTEXT.md §6n | Closes the other §6m gap: turns Spine 2's manual nameserver-pool illustration into a full shared-feature census of the 1,849-member cluster (in-cluster count vs. global degree per feature), then generalises across all 1,334 clusters — only 18/670 (1.3% of all clusters) of clusters with "additional non-org evidence" have that evidence supplied entirely by features with global degree >100 (5/1,334, 0.4%, at >500). |
